...
Inputs
   a, b : rational numbers or NIL.  If both are 
         rational numbers than a <= b.

Output
   c : a rational number.  If a = b = NIL then c = 0.
...
Step1: /* a or b NIL. */
        if (a == NIL) {
           if (b == NIL)
	     c = RNINT(-16); /* originally 0; */
           else
	     c = RNINT(IDIF(RNFLOR(b),16 /* originally 1 */));
           goto Return; }
        else if (b == NIL) {
	  c = RNINT(ISUM(RNCEIL(a),16 /* originally 1 */));
           goto Return; }
