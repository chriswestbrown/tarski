...
TypeExtensionObj::LFH<OpenNuCADObj> OpenNuCADObj::A[] = {
  ...
  {"plot-leaves",&OpenNuCADObj::plotLeaves,"(msg <OpenNuCAD> 'plot-leaves view lab fname) produces an svg-formatted file with name fname that draws the leaf cells of the NuCAD cell with label given by the string lab.  The parameter 'view' is a string describing the portion of the plane to be shown, and the dimensions (in pixels) of the window the plot will appear in.  So, (msg D 'plot-leaves \"-1 3 -2 2 400 400\" \"C\" \"out.svg\") would produce the file out.svg, which would draw cells in the decomposition that fall within -1 < x < 3 and -2 < y < 2.  The plot would be drawn in a 400px-by-400px window.  The lab and fname parameters are optional.  The lab parameter defaults to \"C\", which is the root node.  If the fname parameter is missing, the svg markup will be returned as a string object."},
